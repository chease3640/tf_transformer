...
class Transformer(object):
  ...
  def full_attention_layer(self, hidden_layer):
    with tf.name_scope("full_attention"):
      attention_W = tf.get_variable(name="attention_W",
                                    shape=[self.params.hidden_size, self.params.attention_size], dtype=tf.float32,
                                    initializer=tf.truncated_normal_initializer())
      attention_b = tf.get_variable(name="attention_b",
                                    shape=[self.params.attention_size], dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
      attention_u = tf.get_variable(name="attention_u",
                                    shape=[self.params.attention_size], dtype=tf.float32,
                                    initializer=tf.truncated_normal_initializer())

      output = tf.tanh(tf.matmul(tf.reshape(hidden_layer, [-1, self.params.hidden_size]), attention_W)
                       + tf.reshape(attention_b, [1, -1]))
      output = tf.matmul(output, tf.reshape(attention_u, [-1, 1]))
      output = tf.reshape(tf.exp(output), [-1, self.params.seq_max_len])
      alphas = output / tf.reshape(tf.reduce_sum(output, 1), [-1, 1])

      output = tf.reduce_sum(hidden_layer * tf.reshape(alphas,
                                                       [-1, self.params.seq_max_len, 1]), 1)
